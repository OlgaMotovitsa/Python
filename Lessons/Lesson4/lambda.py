...
add_two_lambda = lambda a, b: a + b # ТАК НЕ НАДО
...
s_value = sorted(my_dict.items(), key=lambda x: x[1]) # ПЕРЕДАЮ АНОНИМНУЮ ФУНКЦИЮ, КОТОРАЯ НА ВХОД ПРИНИМАЕТ НЕКИЙ ОБЪЕКТ,
                                                    # У СЛОВАРЯ МЕТОТ АЙТЕМС ПЕРЕДАЕТ ПАРУ : КЛЮЧ И ЗНАЧЕНИЕ
                                                        # И Х ЭТО КАК РАЗ ПАРА  КЛЮЧ ЗНАЧЕНИЕ, КЛЮЧ В 0 ЯЧЕЙКЕ И ЗНАЧЕНИЕ В 1
print(f'{s_key = }\n{s_value = }')
